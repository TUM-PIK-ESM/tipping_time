...
for (filepath in list.files("SamplePathData", full.names = TRUE)){ ## each synthetic model has data stored in a separate folder
  ...
  crossval_nsim = floor(nsim/10) ## Do this for many sample time series to later adopt the mean of these estimations
  estim.alpha0 = numeric(crossval_nsim)
  estim.mu0 = numeric(crossval_nsim)
  estim.s2 = numeric(crossval_nsim)
  estim.tau = numeric(crossval_nsim)
  estim.a = numeric(crossval_nsim)
  ...
  alpha0_pen0 = mean(estim.alpha0)
  mu0_pen0 = mean(estim.mu0)
  s2_pen0 = mean(estim.s2)
  tau_pen0 = mean(estim.tau) ## This value is the fitting target for the subsequent search of an optimal pen
  a_pen0 = mean(estim.a)
  m_pen0 = mean(estim.mu0 - estim.alpha0/(2*estim.a))
  lambda0_pen0 = mean(- estim.alpha0^2/(4*estim.a))
  tc_pen0 = mean(estim.tau + t0)
