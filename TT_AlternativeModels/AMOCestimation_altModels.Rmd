...
for (folder in list.dirs("SamplePathData", recursive = FALSE)){ ## each synthetic model has data stored in a separate folder
#for (folder in c("SamplePathData/ou_rn_linforc")){
  modelname = substr(folder,start=16,stop=1000000L) 
  nsim = 1000   ## Number of simulations
  n = (2020-1870)*12
  Delta_t = 1/12
  xx = matrix(NA, n, nsim)
  for (isim in 1:nsim){
    xxxx = scan(paste(folder,"/",isim,".txt", sep = ""), what = double(), quiet = TRUE)
    nxx = length(xxxx)
    xx[1:nxx, isim] = xxxx
  }
  ...
  nsim_crossval = 100 ## Do this for many sample time series to later adopt the mean of these estimations
  estim.tau = numeric(nsim_crossval) ## Vector to hold estimates
  for(isim in 1:nsim_crossval){
  ...
  tau_p0 = mean(estim.tau) ## This value is the fitting target for the subsequent search of an optimal penalization p
  
  
  
  
  p.vec = seq(0,0.01,0.001) ## Try this range of values for p. For the AMOC time series in DD23, 0.004 was the optimal value.
  MSE = matrix(0,ncol = 2, nrow = length(p.vec))
  dimnames(MSE)[[2]] = c("p","MSE")
  MSE[,1] = p.vec
  for(i in 1:length(p.vec)){ ## For every choice of p, determine the distribution of estimated tau_r
    pen = p.vec[i]
    estim.tau = numeric(nsim_crossval) ## Vector to hold estimates
   
    for(isim in 1:nsim_crossval){
      xxi   = xx.data[xx.data$time > t0 & xx.data$repetition == isim,"X"]  ##Get data from isim simulation
      xxi.0 = xx.data[xx.data$time <= t0 & xx.data$repetition == isim,"X"] ##Get data from isim simulation
      temp1 = estimate.OU(data = xxi.0, delta = Delta_t)
      temp2 = estimate.tipping(data = xxi, delta = Delta_t, initial.values = c(100, 1), 
                alpha0 = coef(temp1)[1], mu0 = coef(temp1)[2], sigma20 = coef(temp1)[3], pen = pen)
      estim.tau[isim] = temp2$par["tau"]
    }
    MSE[i,2] = mean((estim.tau-tau_p0)^2) ## The distributions are assessed in their square error w.r.t. the fitting target at p = 0
  }
  
  
  
  ## Do full tipping time analysis on all time series with optimal value of penalization p
  pen = p.vec[which.min(c(MSE[,2]))] ## p is chosen as the value where the above square error was minimal
  print(pen)
...
for (folder in list.dirs("SamplePathData", recursive = FALSE)){ ## each synthetic model has data stored in a separate folder
  ...
  take_sample = 1 ## check the goodness of fit for several sample time series, i.e. =2, =3...
  n = (2020-1870)*12
  Delta_t = 1/12
  xx = matrix(NA, n, nsim)
  
  estim.matrix = data.matrix(read_xlsx(paste("EstimMatrix_",modelname,".xlsx", sep = ""))) ## get the estiamtion values of the above analysis
  
  ## read the desired sample path
  xxxx = scan(paste("SamplePathData/",modelname,"/",take_sample,".txt", sep = ""), what = double(), quiet = TRUE)
  nxx = length(xxxx)
  xx[1:nxx, take_sample] = xxxx
